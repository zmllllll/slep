...
impl<'a> Generator<'a, Option<Updater>> for Command<GeneralAction<Postgres, StreamSettings>> {
...
impl<'a> Generator<'a, Result<Receiver>> for Command<GeneralAction<Postgres, StreamSettings>> {
        ...
        match &self.action {
            ...
            | GeneralAction::Upsert { id, resource: _ } => {
                if let Some((_, gid)) = id {
                    ...
                    Err(anyhow::anyhow!("[generate StreamSettings] id is none"))
                }
            }
            _ => Ok(Receiver::None),
        }
