...
//     }
// }
#![feature(async_closure, async_fn_in_trait, associated_type_bounds, let_chains)]
...
pub trait Resource<DB: SqlxDatabase>: GenResourceID<Target = Self::ResourceID> + Serialize {
    ...
    async fn drop<'e, 'c: 'e, E>(id: &Self::ResourceID, executor: E) -> Result<()>
    where
        E: SqlxExecutor<'c, Database = Any>;
}
...
pub enum GeneralAction<DB: SqlxDatabase, R: Resource<DB>> {
    ...
    Update {
        id: R::ResourceID,
        resource: R,
        // update_resource: R::UpdateResource,
    },
    Drop(R::ResourceID),
}
...
{
    #[allow(dead_code)]
    pub async fn execute<'e, 'c: 'e>(&'e self, pool: &'c sqlx::Pool<sqlx::Any>) -> Result<()> {
        match self {
            Commands::Single(cmd) => {
